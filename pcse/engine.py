...
class Engine(BaseEngine):
    ...
    **Signals handled by Engine:**
    ...
    def get_flags(self):
        return np.array([
            self.flag_terminate,
            self.flag_crop_finish,
            self.flag_crop_start,
            self.flag_crop_delete,
            self.flag_output,
            self.flag_summary_output
        ],dtype=np.bool_)
    
    def set_flags(self, flag_arr):
        self.flag_terminate = bool(flag_arr[0])
        self.flag_crop_finish = bool(flag_arr[1])
        self.flag_crop_start = bool(flag_arr[2])
        self.flag_crop_delete = bool(flag_arr[3])
        self.flag_output = bool(flag_arr[4])
        self.flag_summary_output = bool(flag_arr[5])

    # Return the states of the model
    def get_model_state(self):
        if self.crop is None:
            return {'crop':None, 'soil':None, 'pheno':None, 'day':None}
        else: 
            return {'crop':self.crop.get_states(), 'soil':self.soil.get_states(), 'pheno':self.crop.pheno.get_states(), 'day':self.day, 'flags':self.get_flags()}
        
    # Set the states of the model 
    def set_model_state(self, state):
        self.crop.set_states(state['crop'])
        self.soil.set_states(state['soil'])
        self.crop.pheno.set_states(state['pheno'])
        self.day = state['day']
        self.timer.current_date = state['day']
