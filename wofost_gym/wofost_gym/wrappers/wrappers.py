    ...
    def __init__(self, env: gym.Env):
        ...
        if isinstance(self.env.unwrapped, Harvest_NPK_Env):
            if isinstance(env, Harvest_PP_Env):
                ...
                self.action_space = gym.spaces.Dict({"null": Discrete(1), \
                                 "plant": Discrete(1), "harvest": Discrete(1), \
                                 "irrig": Discrete(self.env.unwrapped.num_irrig)})
            elif isinstance(self.env.unwrapped, Harvest_Limited_NPKW_Env): 
                self.action_space = gym.spaces.Dict({"null": Discrete(1), \
                                 "plant": Discrete(1), "harvest": Discrete(1), \
                                 "n": Discrete(self.env.unwrapped.num_fert),\
                                 "p": Discrete(self.env.unwrapped.num_fert),\
        ...
        # Default environments
            ...
            if isinstance(self.env.unwrapped, PP_Env):
                ...
                self.action_space = gym.spaces.Dict({"null": Discrete(1),\
                                 "irrig": Discrete(self.env.unwrapped.num_irrig)})
            elif isinstance(self.env.unwrapped, Limited_NPKW_Env): 
                self.action_space = gym.spaces.Dict({"null": Discrete(1),\
                                 "n": Discrete(self.env.unwrapped.num_fert),\
                                 "p": Discrete(self.env.unwrapped.num_fert),\
                                 "k": Discrete(self.env.unwrapped.num_fert),\
...
class RewardFertilizationCostWrapper(RewardWrapper):
    ...
    def step(self, action: int):
        ...
        truncation = self.env.unwrapped.date >= self.env.unwrapped.crop_end_date
